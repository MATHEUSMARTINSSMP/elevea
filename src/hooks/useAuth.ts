...
export function useAuth() {
  ...
  // Função para carregar e validar usuário
  const loadUser = async () => {
    try {
      setLoading(true);
      setError(null);
      
      // Primeiro tenta ler do localStorage IMEDIATAMENTE (sem delay)
      const storedUser = readAuthFromStorage();
      if (storedUser && storedUser.email) {
        console.log("🔍 useAuth: Usuário encontrado no localStorage", storedUser);
        
        // Se tem email e role, já pode usar (mesmo sem siteSlug completo)
        // O siteSlug pode vir da URL após redirect do Google Auth
        if (storedUser.email && storedUser.role) {
          setUser(storedUser);
          setError(null);
          setLoading(false);
          
          // Se não tem siteSlug mas tem email, tentar buscar do n8n em background
          if (!storedUser.siteSlug) {
            console.log("🔍 useAuth: Sem siteSlug, buscando do n8n em background...");
            // Continuar para buscar do n8n, mas não bloquear a UI
          } else {
            return; // Tem tudo, pode retornar
          }
        }
  ...
  useEffect(() => {
    ...
    // Listener para mudanças no localStorage (quando outros componentes atualizam)
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === "auth" || e.key === "elevea_last_email") {
        console.log("🔍 useAuth: localStorage mudou, recarregando usuário...");
        if (alive) {
          loadUser();
        }
      }
    ...
    // Também verificar mudanças periódicas (para mudanças no mesmo tab)
    const intervalId = setInterval(() => {
      if (alive) {
        const currentAuth = localStorage.getItem("auth");
        const lastAuth = sessionStorage.getItem("last_auth_check");
        if (currentAuth !== lastAuth) {
          sessionStorage.setItem("last_auth_check", currentAuth || "");
          console.log("🔍 useAuth: Mudança detectada no localStorage, recarregando...");
          loadUser();
        }
      }
